...
function networkUp() {
  ...
  generateIdentityForOrganization factory
  generateIdentityForOrganization dealer
  generateIdentityForOrganization police
  generateIdentityForOrganization insurance
  ...
  generateAnchorPeerUpdate Global global factory
  generateAnchorPeerUpdate Global global dealer
  generateAnchorPeerUpdate Global global police
  generateAnchorPeerUpdate Global global insurance
...
function runScenario() {
    docker exec cli.factory.example.com scripts/01_create_new_car.sh $CLI_DELAY $CLI_TIMEOUT $VERBOSE

    sleep 3

    docker exec cli.police.example.com scripts/02_change_owner.sh $CLI_DELAY $CLI_TIMEOUT $VERBOSE

    sleep 3

    docker exec cli.insurance.example.com scripts/03_set_inactive.sh $CLI_DELAY $CLI_TIMEOUT $VERBOSE

    sleep 3

    docker exec cli.police.example.com scripts/04_change_owner_for_inactive.sh $CLI_DELAY $CLI_TIMEOUT $VERBOSE
}
