...
lifecycleCommitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o orderer.example.com:7050 --tls --cafile "${ORDERER_CA}" --channelID "${channel}" --name "${name}" --version "${version}" --sequence 1 $PEER_CONN_PARAMS >&log.txt
  res=$?
  set +x
...
joinChannel global factory
joinChannel global dealer
joinChannel global police
joinChannel global insurance
echo ">>> done..."
...
updateAnchorPeers global factory
updateAnchorPeers global dealer
updateAnchorPeers global police
updateAnchorPeers global insurance
echo ">>> done..."

echo ">>> packaging 'history' chaincode..."
# chaincode_name version org
packageChaincode history 1 auditor
# chaincode_name org version
installChaincode history auditor 1
installChaincode history factory 1
installChaincode history dealer 1
installChaincode history police 1
installChaincode history insurance 1
# org
queryInstalled auditor
# channel_name chaincode_version org version
approveChaincode global history auditor 1
approveChaincode global history factory 1
approveChaincode global history dealer 1
approveChaincode global history police 1
approveChaincode global history insurance 1
# channel_name chaincode_name org version @peers
lifecycleCommitChaincodeDefinition global history auditor 1 auditor factory dealer police insurance
# channel_name chaincode_name org args @peers
chaincodeInvoke global history auditor '{"function":"InitLedger","Args":[]}' auditor factory dealer police insurance
